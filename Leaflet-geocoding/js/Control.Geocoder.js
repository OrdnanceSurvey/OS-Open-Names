...
///////////////////////////
//
// OS Open Names extension to be integrated at later date when it doesn't require Proj4
// 
///////////////////////////
			...
			L.Control.Geocoder.getJSON(this.options.service_url + 'find', params, function(data) {
					...
					if (data.results && data.results.length) {
						for (var i = 0; i <= data.results.length - 1; i++) {
							...
							if (loc.GAZETTEER_ENTRY.POPULATED_PLACE) {
								gaz += loc.GAZETTEER_ENTRY.POPULATED_PLACE += ', ';
							}
							if (loc.GAZETTEER_ENTRY.COUNTY_UNITARY) {
								gaz += loc.GAZETTEER_ENTRY.COUNTY_UNITARY += ', ';
							}
							if (loc.GAZETTEER_ENTRY.REGION) {
								gaz += loc.GAZETTEER_ENTRY.REGION += ', ';
							}
							if (loc.GAZETTEER_ENTRY.COUNTRY) {
								gaz += loc.GAZETTEER_ENTRY.COUNTRY;
							}
							results[i] = {
									name: gaz,
									bbox: latLngBounds,
	...
	L.Control.Geocoder.ordnancesurvey = function(key) {
		return new L.Control.Geocoder.OSOpenNames(key);
	};
